...
namespace subjectnerdagreement.psdexport
{
    public class PSDExporter
    {
        ...
        public static Sprite CreateSprite(PsdExportSettings settings, int layerIndex)
        {
            ...
            if (tex == null)
                return null;
            Sprite sprite = SaveAsset(settings, tex, layerIndex, tex.width, tex.height);
        ...
        private static void createSpritesUsingImageMagickFilterAndSaveInDirectory(int newWidth, int newHeight, string oldFilePath, string newFilePath)
        {
            string fileDirectory = Path.GetDirectoryName(newFilePath) + "/";
            UnityEngine.Debug.Log("FILE Directory" + fileDirectory);
            FileInfo fileInfo = new FileInfo(fileDirectory);
            if (!fileInfo.Exists)
                Directory.CreateDirectory(fileInfo.Directory.FullName);
#if UNITY_EDITOR_WIN
            BiCubic(newWidth, newHeight, oldFilePath, newFilePath);
#endif
#if UNITY_EDITOR_OSX
                MakeImageUsingCommandLine(newWidth, newHeight, assetPath, newFilePath);                
#endif

        }

        static void BiCubic(int width, int height, string path, string newPath)
        {
        ...
        static void MakeImageUsingCommandLine(int nWidth, int nHeight, string assetPath, string newFileName)
        {
            UnityEngine.Debug.Log("terminal regex");
            UnityEngine.Debug.Log(doubleQuotesToHandleSpace(assetPath));
            UnityEngine.Debug.Log(doubleQuotesToHandleSpace(newFileName));
            ExecuteCommand(doubleQuotesToHandleSpace(assetPath) + " -filter Lanczos -distort resize " + nWidth.ToString() + "x" + nHeight.ToString() + " " + doubleQuotesToHandleSpace(newFileName));
        }

        static string doubleQuotesToHandleSpace(string str)
        {
            return "\"" + str + "\"";
        }
        ...
        {
            var thread = new System.Threading.Thread(delegate () { Command("/usr/local/bin/convert", command); });
            thread.Start();

        }

        static void Command(string fileName, string command)
        {
        ...
        private static Sprite SaveAsset(PsdExportSettings settings, Texture2D tex, int layer, int texWidth, int texHieght)
        {
            ...
            // Apply local scaling
            int width = (int)(((float)texWidth + 1.0f) / (float)(scaleBy));
            int height = (int)(((float)texHieght + 1.0f) / (float)(scaleBy));
            if (layerSetting.scaleBy != ScaleDown.Default)
            {
                createSpritesUsingImageMagickFilterAndSaveInDirectory(width, height, assetPath, assetPath);
            }
            ...
            if (settings.ScaleBy > 0)
            {
                ...
                createSpritesUsingImageMagickFilterAndSaveInDirectory(nwidth, nheight, assetPath, assetPath);
            }
            AssetDatabase.Refresh();
